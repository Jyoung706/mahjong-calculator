...
const names = (r: ReturnType<typeof calculateScore>) => r.yaku.map((y) => y.name).sort();
...
test('7역 중첩: 리치+일발+쯔모+핑후+탄야오+이페코+삼색동순 = 8판 배만', () => {
  ...
  expect(names(r)).toEqual(['리치','멘젠쯔모','삼색동순','이페코','일발','탄야오','핑후']);
  expect(r.han).toBe(8);
...
test('량페코 vs 치토이츠: 높은 쪽(량페코 3판 40부)을 채택', () => {
  ...
  expect(names(r)).toEqual(['량페코']); // 치토이츠·이페코로 세지 않음
  expect(r.han).toBe(3);
...
test('더블리치+일발+우라도라 → 5판 만관 (리치 항목은 없어야 함)', () => {
  ...
  expect(names(r)).toEqual(['더블리치','역패 中','우라도라','일발']);
  expect(r.han).toBe(5);
...
test('부로 복합: 發폰 + 白샤보론 + 혼일색 + 찬타 = 5판 만관', () => {
  ...
  expect(names(r)).toEqual(['역패 發','역패 白','찬타','혼일색']);
  expect(r.han).toBe(5);
  expect(r.limitName).toBe('만관');
  expect(r.payment.ron).toBe(8000);
});
...
test('카조에 역만 룰 옵션', () => {
  ...
  expect(pay(true)).toMatchObject({ basePoints: 8000, limitName: '역만' });
  expect(pay(false)).toMatchObject({ basePoints: 6000, limitName: '삼배만' });
});
