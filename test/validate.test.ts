...
test('손패 매수 오류', () => {
  expect(validate(base(hand13.slice(1), '5s'))).toMatch('패 매수 오류');
  expect(validate(base(hand13, '5s', [{ type: 'pon', tiles: ['5z', '5z', '5z'] }]))).toMatch('패 매수 오류');
});

test('같은 패 5장', () => {
  const h: Tile[] = ['1m', '1m', '1m', '1m', '5p', '6p', '7p', '8p', '9p', '7z', '7z', '7z', '1m'];
  expect(validate(base(h, '1m'))).toMatch('같은 패 5장 이상: 1m');
});

test('부로 구성 오류', () => {
  expect(validate(base(hand13.slice(3), '5s', [{ type: 'pon', tiles: ['5z', '5z', '6z'] }]))).toMatch('부로 구성 오류');
  expect(validate(base(hand13.slice(3), '5s', [{ type: 'chi', tiles: ['1s', '2s', '4s'] }]))).toMatch('부로 구성 오류');
  expect(validate(base(hand13.slice(3), '5s', [{ type: 'chi', tiles: ['1z', '2z', '3z'] }]))).toMatch('부로 구성 오류');
  expect(validate(base(hand13.slice(3), '5s', [{ type: 'minkan', tiles: ['5z', '5z', '5z'] }]))).toMatch('부로 매수 오류');
});
